...
`.env.example` 保留为 Docker/生产环境占位模板；本地开发请优先使用 `.env.local.example`，因为主机进程访问数据库需要 `localhost`，而容器内部访问数据库需要 Compose 服务名 `postgres`。

## 安全说明

- 不要提交 `.env`、服务器 IP、密码、SSH 私钥或 API Key。
- 生产环境将 `APP_ENV` 放在 GitHub Secrets，并由 workflow 写入 VPS 的 `.env`。
- 如果部署用的 SSH 私钥带 passphrase，将 passphrase 放入 GitHub Secret `SSH_PASSPHRASE`。
